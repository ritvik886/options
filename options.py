...
def get_historical_options_volume(ticker):
    """Get average weekly options volume - RETURNS 2 VALUES: total_volume, total_premium"""
    try:
        tkr = yf.Ticker(ticker)
        expiries = tkr.options[:3]
...
if scan_btn:
    ...
    if df.empty:
        ...
        # TAB 2: PREMIUM FLOW ANALYSIS - FIXED
        with tab2:
            ...
            for ticker in df['ticker'].unique():
                ...
                # FIXED: Function returns 2 values - total_volume, total_premium
                _, hist_total_prem = get_historical_options_volume(ticker)
                
                if hist_total_prem > 0:
                    prem_multiplier = min(total_prem / hist_total_prem, 100.0)
                else:
                    prem_multiplier = 0
                ...
                premium_data.append({
                    ...
                    'Normal Premium': f"${hist_total_prem:,.0f}",
        ...
        # TAB 3: HISTORICAL CONTEXT - FIXED
        with tab3:
        ...
        with tab5:
            ...
            if not block_trades_df.empty:
                ...
                st.warning("No block trades detected at current thresholds.")import streamlit as st
import yfinance as yf
import pandas as pd
import numpy as np
from datetime import datetime, timedelta
